...
//        customizer.disable() is used to disable csrf token so you don't have to attach it in the headers in Postman.
        return http.csrf(customizer -> customizer.disable()).
                authorizeHttpRequests(request -> request
                        .requestMatchers("login", "register").permitAll() // this makes login and register urls accessible even without authentication
                        .anyRequest().authenticated()) // any other url path needs authentication
//                httpBasic is used to authenticate the user from Postman. In Postman, we have to select Basic Auth and enter the username and password. For the browser it would be formLogin, which is a built-in login screen, but can't do formLogin because when you submit the login, it'll be a new request, which will be another session token, so have to do sessionManagement below. This will create a login popup.
                .httpBasic(Customizer.withDefaults()).
                sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .addFilterBefore(jwtFilter, UsernamePasswordAuthenticationFilter.class) // this makes the jwtFilter first. If jwtFilter passes authentication, it won't need to do UsernamePasswordAuthenticationFilter
                .build();


    }

//    instead of using the default spring.security.user.name and spring.security.user.password, we can use the below code to create our own users.
//    @Bean
